...
from app.models.hotspot import HotspotPortal, RadiusNasDevice, RadiusProfile
...
def ensure_bootstrap_data(db: Session) -> None:
    ...
    if not db.query(AccessPlan).first():
        guest_plan = AccessPlan(plan_name="Guest 1h", download_kbps=2048, upload_kbps=1024, max_duration_minutes=60, idle_timeout_minutes=15, concurrent_sessions=1)
        premium_plan = AccessPlan(plan_name="Premium Day", download_kbps=10240, upload_kbps=5120, max_data_mb=5000, max_duration_minutes=1440, idle_timeout_minutes=30, concurrent_sessions=2)
        db.add_all([guest_plan, premium_plan])
        db.flush()

    if not db.query(RadiusProfile).first():
        plans = db.query(AccessPlan).order_by(AccessPlan.id.asc()).all()
        if plans:
            profiles = []
            for index, plan in enumerate(plans):
                profiles.append(
                    RadiusProfile(
                        ...
                        profile_name=plan.plan_name.lower().replace(" ", "-"),
                        radius_group_name=plan.plan_name.lower().replace(" ", "-"),
    ...
    if not db.query(HotspotPortal).first():
        db.add(
            HotspotPortal(
                portal_name="default",
                portal_host="portal.tinnicore.local",
                landing_path="/login",
                success_path="/status",
    ...
    if not db.query(RadiusNasDevice).first():
        db.add(
            RadiusNasDevice(
                name="advgate-local",
