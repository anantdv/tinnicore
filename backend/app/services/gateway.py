...
class GatewayService:
    ...
    @staticmethod
    def _portal_server_settings(portal: HotspotPortal | None) -> dict[str, Any]:
        defaults = {
            "service": "enable",
        ...
        return {**defaults, **((portal.server_settings if portal else None) or {})}
    ...
    def render_chilli_config(self) -> RenderedConfig:
        ...
        server_settings = self._portal_server_settings(portal)
        service_enabled = server_settings.get("service") == "enable"
        server_address = str(server_settings.get("web_server_address") or "auto_detect")
        server_port = str(server_settings.get("web_server_port") or "2280")
        if server_settings.get("captive_portal") == "external_web_server" and server_address not in {"", "auto_detect"}:
            portal_base_url = f"http://{server_address}:{server_port}".rstrip("/")
        else:
            portal_base_url = settings.portal_base_url.rstrip("/")
        portal_url = f"{portal_base_url}{portal.landing_path if portal else '/login'}"
        redirect_target = server_settings.get("redirect_url_after_login")
        ...
        content = textwrap.dedent(
            f"""
            # Auto-generated by TINNICORE
            # Service: {"enabled" if service_enabled else "disabled"}
            ...
            HS_UAMFORMAT={portal_url}
            HS_UAMHOMEPAGE={portal_base_url}/
            HS_UAMSECRET={settings.secret_key}
            HS_NASID={server_settings.get("nas_identifier")}
            HS_UAMPORT={server_port}
            HS_UAMLISTEN={lan_ip if server_address == "auto_detect" else server_address}
            HS_LOGINMODE={server_settings.get("authentication_method")}
            HS_REDIRECT={redirect_target}
            HS_ACCOUNT_STATUS={server_settings.get("account_status_after_login")}
            HS_LOGOUT_IP={server_settings.get("logout_shortcut_ip")}
            HS_DNS1=1.1.1.1
            HS_DNS2=8.8.8.8
